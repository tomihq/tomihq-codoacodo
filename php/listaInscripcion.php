...
<?php
     ...
     function updateUser($id){
        ...
        $fields = "name='".$_POST["data"][0]."', surname='".$_POST["data"][1]."', email='".$_POST["data"][2]."'";

        try {
            $stmt = $mysqli -> prepare("SELECT email FROM person WHERE email=?");
            $stmt -> bind_param("s", trim($_POST["data"][2]));
            $stmt->execute();
            $email = $stmt->get_result();
            echo json_encode(array("ok"=>true, "msg" => $email));
            exit;
            if(is_null($stmt)){
     ...
     if(isset($_POST["id"]) && $_POST["method"] === 'update') updateUser($_POST["id"], $_POST["data"]);
    ...
    <main class="main-container p-4">
        ...
        ?>
                     <tr>
                        ...
                        <td> 
                            <div>
                                <button class="btn btn-primary modifyUser" id="<?php echo $person["id"] ?>" >Modificar</a></button>
